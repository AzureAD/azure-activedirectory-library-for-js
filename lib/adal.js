...
var AuthenticationContext = (function () {
    ...
    */
    ...
    AuthenticationContext.prototype.acquireToken = function (resource, callback) {
        ...
        // if there is no logged in user or either a login_hint or sid is provided 
        // the user must login.
        if (!this._user &&
            !(this.config.extraQueryParameter && 
            (this.config.extraQueryParameter.indexOf('login_hint') !== -1 ||
            this.config.extraQueryParameter.indexOf('sid') !== -1))) {
  ...
  */
    AuthenticationContext.prototype.acquireTokenPopup = function (resource, extraQueryParameters, claims, callback) {
        ...
        // check for user exists if not they are not logged in or if the session id (sid) exists allow
        // since single sign on was in progress and needed an interactive response
        if (!this._user &&
            !(this.config.extraQueryParameter && this.config.extraQueryParameter.indexOf('sid') !== -1)) {
            this.warn('User login is required');
            callback('User login is required', null, 'login required');
            return;
        }
        ...
        // also remove sid if it exists since its not compatible with prompt=select_account
        var urlNavigate = this._urlRemoveQueryStringParameter(this._getNavigateUrl('token', resource), 'prompt');
        urlNavigate = this._urlRemoveQueryStringParameter(urlNavigate, 'sid');
    ...
    AuthenticationContext.prototype._addHintParameters = function (urlNavigate) {
        //If you do not use prompt=none, then if the session does not exist, there will be a failure.
        ...
        if (this._user && this._user.profile) {
            if (this._user.profile.sid && urlNavigate.indexOf('&prompt=none') !== -1) {
                ...
                if (!this._urlContainsQueryStringParameter("sid", urlNavigate) && !this._urlContainsQueryStringParameter("login_hint", urlNavigate)) {
                    // add sid
