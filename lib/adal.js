...
AuthenticationContext.prototype.getCachedToken = function (scopes) {
    ...
    var valueInCache = JSON.parse(this._getItem(targetEntryKey));
    if (valueInCache && this._scopeContains(valueInCache[this.CONSTANTS.PARAMETERS.SCOPE], scopes)) {
        
        var entryKey = this._createTokenCacheEntryKey();
        var value = JSON.parse(this._getItem(entryKey));
...
AuthenticationContext.prototype._renewIdToken = function (callback) {
    ...
    if (!this._hasEntry(this.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY + this.config.clientId)) {
        var keys = JSON.parse(this._getItem(this.CONSTANTS.STORAGE.TOKEN_KEYS)) || [];
        keys.push(this.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY + this.config.clientId);
        this._saveItem(this.CONSTANTS.STORAGE.TOKEN_KEYS, JSON.stringify(keys));
    }
                ...
                if (this._user && this._user.profile) {
                    if (this._user.profile.nonce !== this._getItem(this.CONSTANTS.STORAGE.NONCE_IDTOKEN)) {
                        ...
                        if (!this._hasEntry(entry)) {
                            ...
                            console.log('For testing to log ' + JSON.stringify(keys));
                            this._saveItem(this.CONSTANTS.STORAGE.TOKEN_KEYS, JSON.stringify(keys));
                        }
