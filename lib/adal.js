...
var AuthenticationContext = (function () {
    ...
    */
    ...
    AuthenticationContext.prototype.getCachedUserAsync = function(callback) {
        ...
        // check for msal idtoken
        var rawIdToken = this._getItem(this.CONSTANTS.STORAGE.MSAL_IDTOKEN);
        if (!this._isEmpty(rawIdToken)) {
            var idtoken = this._extractIdToken(rawIdToken);
            var upn = idtoken.preferred_username;
            var self = this;
            if (!this._isEmpty(upn)) {
                this._acquireIdTokenSilent(upn, function(errorDesc, idtoken, error) {
                    this._user = self._createUser(idtoken);
                    callback(this._user);
                });
            } else {
                callback(null);
    ...
    AuthenticationContext.prototype.acquireToken = function (resource, callback) {
        ...
        var self = this;
        this.getCachedUserAsync(function(user){
            if (!user && !(self.config.extraQueryParameter && self.config.extraQueryParameter.indexOf('login_hint') !== -1)) {
                self.warn('User login is required');
            ...
            if (self._activeRenewals[resource]) {
                ...
                self.registerCallback(self._activeRenewals[resource], resource, callback);
            }
            else {
                self._requestType = self.REQUEST_TYPE.RENEW_TOKEN;
                if (resource === self.config.clientId) {
                    // App uses idtoken to send to api endpoints
                    // Default resource is tracked as clientid to store this token
                    if (user) {
                        self.verbose('renewing idtoken');
                        self._renewIdToken(callback);
                    }
                    else {
                        self.verbose('renewing idtoken and access_token');
                        self._renewIdToken(callback, self.RESPONSE_TYPE.ID_TOKEN_TOKEN);
                    }
                } else {
                    if (user) {
                        self.verbose('renewing access_token');
                        self._renewToken(resource, callback);
                    }
                    else {
                        self.verbose('renewing idtoken and access_token');
                        self._renewToken(resource, callback, self.RESPONSE_TYPE.ID_TOKEN_TOKEN);
                    }
                }
            }
        });
  ...
  */
    AuthenticationContext.prototype.acquireTokenPopup = function (resource, extraQueryParameters, claims, callback) {
        ...
        this.getCachedUserAsync(function(user) {
            ...
            if (self._acquireTokenInProgress) {
                self.warn("Acquire token interactive is already in progress")
                callback("Acquire token interactive is already in progress", null, "Acquire token interactive is already in progress");
                return;
            }
    
            var expectedState = self._guid() + '|' + resource;
            self.config.state = expectedState;
            self._renewStates.push(expectedState);
            self._requestType = self.REQUEST_TYPE.RENEW_TOKEN;
            self.verbose('Renew token Expected state: ' + expectedState);
            // remove the existing prompt=... query parameter and add prompt=select_account
            var urlNavigate = self._urlRemoveQueryStringParameter(self._getNavigateUrl('token', resource), 'prompt');
            ...
            urlNavigate = self._addHintParameters(urlNavigate);
            self._acquireTokenInProgress = true;
            self.info('acquireToken interactive is called for the resource ' + resource);
            self.registerCallback(expectedState, resource, callback);
            self._loginPopup(urlNavigate, resource, callback);
        });
    };
    ...
    AuthenticationContext.prototype.acquireTokenRedirect = function (resource, extraQueryParameters, claims) {
        ...
        this.getCachedUserAsync(function(user){
            ...
            self._acquireTokenInProgress = true;
            self.info('acquireToken interactive is called for the resource ' + resource);
            self._saveItem(self.CONSTANTS.STORAGE.LOGIN_REQUEST, window.location.href);
            self._saveItem(self.CONSTANTS.STORAGE.STATE_RENEW, expectedState, true);
            self.promptUser(urlNavigate);
        });
    };
    /**
    ...
    */
    ...
    AuthenticationContext.prototype._acquireIdTokenSilent = function (upn, callback) {
        ...
        if (this._activeRenewals[resource]) {
            ...
            // add login_hint
            urlNavigate += '&login_hint=' + encodeURIComponent(upn);
