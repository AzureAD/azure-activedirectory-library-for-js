...
var AuthenticationContext = (function () {
     ...
     *  @property {function} displayCall - User defined function of handling the navigation to Azure AD authorization endpoint. Defaults to 'null'.
    ...
    AuthenticationContext.prototype._getEndpointByResource = function(resource) {
        ...
        if (this.config && this.config.endpoints) {
            for (var configEndpoint in this.config.endpoints) {
                var endpoint = this.config.endpoints[configEndpoint];
        ...
        if (parameters) {
            ...
            if (parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION) ||
                ...
                if (parameters.hasOwnProperty('state')) {
                    stateResponse = decodeURIComponent(parameters.state);
                    this.verbose('State: ' + stateResponse);
                } else {
        ...
        if (this.config && this.config.endpoints) {
            for (var configEndpoint in this.config.endpoints) {
                ...
                if (endpoint.indexOf(configEndpoint) > -1) {
                    var matchingEndpoint = this.config.endpoints[configEndpoint];
