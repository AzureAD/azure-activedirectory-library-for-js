...
var AuthenticationContext = (function () {
    ...
    AuthenticationContext = function (config) {
        ...
        // by default we need to move to LOGIN_REQUEST url after window callback handling.
    ...
    AuthenticationContext.prototype.handleWindowCallback = function (hash) {
        ...
        if (this.isCallback(hash)) {
            ...
            if (!this.popUp) {
                var urlWithoutHash = window.location.href.indexOf('#') != -1
                    ? window.location.href.split("#")[0]
                    : window.location.href;

                if (this.config.navigateToLoginRequestUrl && urlWithoutHash !== this._getItem(this.CONSTANTS.STORAGE.LOGIN_REQUEST)) {
