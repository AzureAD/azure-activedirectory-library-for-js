...
AuthenticationContext.prototype._clearRenewState = function () {
...
AuthenticationContext.prototype._cancelOutstandingRequests = function () {

    this.warn('Canceling outstanding requests');

    //Remove the id frame
    this._removeAdalFrame('adalIdTokenFrame');

    //Remove all resource frames
    for (var resource in this._activeRenewals) {
        if (this._activeRenewals.hasOwnProperty(resource)) {
            var frameName = 'adalRenewFrame' + resource;
            this._removeAdalFrame(frameName);
        }
    }
...
AuthenticationContext.prototype._removeAdalFrame = function (iframeId) {
    if (typeof iframeId === 'undefined') {
        return;
    }

    this.info('Remove adal frame from document:' + iframeId);
    var adalFrame = document.getElementById(iframeId);

    if (adalFrame) {
        if (document.removeChild && document.documentElement &&
            (window.opera || window.navigator.userAgent.indexOf('MSIE 5.0') === -1)) {
            document.getElementsByTagName('body')[0].removeChild(adalFrame);
        }
    }
}
