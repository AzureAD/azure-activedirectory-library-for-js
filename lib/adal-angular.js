                ...
                var routeChangeHandler = function (e, nextRoute) {
                    if (nextRoute && nextRoute.$$route) {
                        if (isADLoginRequired(nextRoute.$$route, _adal.config)) {
                            ...
                            if (nextRoute.$$route.templateUrl) {
                                var urlNavigate = '';
                                if (typeof nextRoute.$$route.templateUrl === 'function') {
                                    urlNavigate = nextRoute.$$route.templateUrl(nextRoute.params);
                                } else {
                                    urlNavigate = nextRoute.$$route.templateUrl;
                                }
                                if (urlNavigate !== '' && !isAnonymousEndpoint(urlNavigate)) {
                                    _adal.config.anonymousEndpoints.push(urlNavigate);
                                }
                            }
                        }
                    }
                        ...
                        if (toState.hasOwnProperty('parent')) {
                            ...
                            for (var i = 0; i < states.length; i++) {
                                ...
                                if (isADLoginRequired(state, _adal.config)) {
                                    ...
                                    if (typeof state.templateUrl === 'function')
                                        urlNavigate = state.templateUrl(toParams);
                                    else
                                        urlNavigate = state.templateUrl;
                                    if (urlNavigate !== '' && !isAnonymousEndpoint(urlNavigate))
                                        _adal.config.anonymousEndpoints.push(urlNavigate);
                                }
                            }
                            ...
                            for (var i = 0; i < states.length; i++) {
                                if (i == 0)
                                    ...
                                    state_name += '.' + states[i];
                                state = $injector.get('$state').get(state_name);
                                if (state && isADLoginRequired(state, _adal.config)) {
                                    if (!_oauthData.isAuthenticated) {
                                        if (!_adal._renewActive && !_adal.loginInProgress()) {
                                            _adal.info('State change event for:' + $location.$$url);
                                            loginHandler();
                                ...
                                else if (state && state.templateUrl) {
                                    var urlNavigate = '';
                                    if (typeof state.templateUrl === 'function')
                                        urlNavigate = state.templateUrl(toParams);
                                    else
