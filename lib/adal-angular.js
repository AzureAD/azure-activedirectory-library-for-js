            ...
            this.$get = ['$rootScope', '$window', '$q', '$location', '$timeout', function ($rootScope, $window, $q, $location, $timeout) {
                            ...
                            if (!_adal._getItem(_adal.CONSTANTS.STORAGE.FAILED_RENEW)) {
                                ...
                                _adal.acquireToken(_adal.config.loginResource, function (error, tokenOut) {
                                    if (error) {
                                        ...
                                        $rootScope.$broadcast('adal:tokenRenewFailure', 'auto renew failure ' + error, _adal.config.loginResource);
                                    } else {
                                        _isRenewingToken = false;
                                        if (tokenOut) {
                                            _oauthData.isAuthenticated = true;
                                            $rootScope.$broadcast('adal:tokenRenewSuccess', _adal.config.loginResource);
                                        }
                                    }
                                });
                            }
                ...
                var isInIFrame = function () {
                    try {
                        ...
                        console && console.error('isInIFrame: Unexpected error:', e);
                        return true;
                    }
                }
                ...
                var routeChangeHandler = function (e, nextRoute) {
                    if (nextRoute && nextRoute.$$route && isADLoginRequired(nextRoute.$$route, _adal.config)) {
                        if (!_oauthData.isAuthenticated && !isInIFrame() && !_isRenewingToken) {
                ...
                return {
                    ...
                    acquireToken: function (resource) {
                        ...
                        _cachedTokens[resource]= _adal.getCachedToken(resource);
                        _adal.acquireToken(resource, function (error, tokenOut) {
                            if (error) {
                                _adal.error('Error when acquiring token for resource: ' + resource, error);
                                $rootScope.$broadcast('adal:tokenRenewFailure', 'auto renew failure ' + error, resource);
                                deferred.reject(error);
                            } else {
                                deferred.resolve(tokenOut);
                                if (_cachedTokens[resource] !== tokenOut) {
                                    $rootScope.$broadcast('adal:tokenRenewSuccess', resource);
                                    _cachedTokens[resource] = tokenOut;
                                }
                            }
                        });
