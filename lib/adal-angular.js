...
(function () {
    ...
    if (angular) {
        ...
        AdalModule.provider('adalAuthenticationService', function () {
            ...
            var updateDataFromCache = function (scopes, policy) {
                ...
                var token = _adal.getCachedToken(scopes, policy);
            ...
            this.$get = ['$rootScope', '$window', '$q', '$location', '$timeout', function ($rootScope, $window, $q, $location, $timeout) {
                    ...
                    var locationChangeHandler = function () {
                        ...
                        if (_adal.isCallback(hash)) {
                            ...
                            Logging.log('No callback. App resumes after closing or moving to new page.');         
                            updateDataFromCache(_adal.config.clientId);
                    ...
                    var loginHandler = function (policy) {
                    ...
                    var routeChangeHandler = function (e, nextRoute) {
                        if (nextRoute && nextRoute.$$route && isADLoginRequired(nextRoute.$$route, _adal.config)) {
                            if (!_oauthData.isAuthenticated) {
                                ...
                                var policy = nextRoute.$$route.requireADLogin !== true ? nextRoute.$$route.requireADLogin : '';
                                loginHandler(policy);
                            }
                        }
                    };
                    ...
                    var stateChangeHandler = function (e, nextRoute) {
                        if (nextRoute && isADLoginRequired(nextRoute, _adal.config)) {
                            if (!_oauthData.isAuthenticated) {
                                ...
                                var policy = nextRoute.requireADLogin !== true ? nextRoute.requireADLogin : '';
                                loginHandler(policy);
                            }
                        }
                    };
                    ...
                    return {
                        ...
                        config: _adal.config,
                        login: function (scopes, policy) {
                            _adal.login(scopes, policy);
                        },
                        ...
                        getCachedToken: function (scopes, policy) {
                            return _adal.getCachedToken(scopes, policy);
                        },
                        userInfo: _oauthData,
                        acquireTokenSilent: function (scopes, policy) {
                            // automated token request call
                            var deferred = $q.defer();
                            _adal.acquireTokenSilent(scopes, policy, function (error, tokenOut) {
                                if (error) {
                        ...
                        clearCacheForStoredEntry: function (scope, policy) {
                            _adal.clearCacheForStoredEntry(scope, policy);
                        }
                    };
                }];
        });
                ...
                return {
                    request: function (config) {
                        if (config) {
                            ...
                            var policy = authService.getPolicyForEndpoint(config.url);
                            
                            var tokenStored = authService.getCachedToken(scope, policy);
                            var isEndpoint = false;
                            if (tokenStored) {
                                Logging.log('Token is avaliable for this url ' + config.url);
                                ...
                                if (authService.loginInProgress()) {
                                    Logging.log('login already start.');
                                    return $q.reject();
                                } else if (authService.config && isEndpoint) {
                                    // external endpoints
                                    // delayed request to return after iframe completes
                                    var delayedRequest = $q.defer();
                                    authService.acquireTokenSilent(scope, policy).then(function (token) {
                                        Logging.log('Token is available.');
                    ...
                    responseError: function (rejection) {
                        Logging.log('Getting error in the response');
                        if (rejection && rejection.status === 401) {
                            var scopes = authService.getScopesForEndpoint(rejection.config.url);
                            var policy = authService.getScopesForEndpoint(rejection.config.url);
                            
                            authService.clearCacheForStoredEntry(scopes, policy);
