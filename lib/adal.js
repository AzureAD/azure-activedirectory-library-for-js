...
var AuthenticationContext = (function () {
    ...
    AuthenticationContext.prototype._loadFrameTimeout = function (urlNavigation, frameName, resource) {
        ...
        var timeoutDuration = this.CONSTANTS.LOADFRAME_TIMEOUT / 1000;
        var self = this;
        setTimeout(function () {
            if (self._getItem(this.CONSTANTS.STORAGE.RENEW_STATUS + resource) === self.CONSTANTS.TOKEN_RENEW_STATUS_IN_PROGRESS) {
                //fail the iframe session if it's in pending state
                self.verbose('Loading frame has timed out after: ' + timeoutDuration + ' seconds for resource ' + resource);
                ...
                this._saveItem(this.CONSTANTS.STORAGE.RENEW_STATUS + resource, '');
            }
