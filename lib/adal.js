...
// Dummy code for Oauth2 implicit flow to get access token only
...
function Adal(config) {
    ...
    this.startPage = null;
    this.user = null;
    this.frameRequestType = this.REQUEST_TYPE.UNKNOWN;
};
module.export = Adal;
Adal.prototype.setStartPage = function(start){
	this.startPage = start;
};

Adal.prototype.REQUEST_TYPE = {
...
Adal.prototype._renewToken = function (resource, callback) {
    ...
    this.callback = callback;
    this.frameRequestType = this.REQUEST_TYPE.ACCESS_TOKEN;
...
Adal.prototype._getIdTokenInFrame = function(callback) {
    ...
    this.callback = callback;
    this.frameRequestType = this.REQUEST_TYPE.ID_TOKEN;
    logStatus("Navigate to:" + urlNavigate);
    frameHandle.src = urlNavigate;
};

Adal.prototype.oauth2Callback = function (hash, iframeRequest) {
    var parameters = this._deserialize(hash.substring(1));
    ...
    if (parameters.hasOwnProperty("error_description")) {
        ...
        if (iframeRequest && this.frameRequestType == this.REQUEST_TYPE.ACCESS_TOKEN) {
        ...
        for (var p in parameters) {
            logStatus("Param p:" + p + " value:" + parameters[p]);
        }
    }
    ...
    if (typeof this.callback === 'function') {
        ...
        if (this.frameRequestType == this.REQUEST_TYPE.ACCESS_TOKEN) {
...
//if (window.addEventListener) window.addEventListener("load", addAdalFrame, false);
//else if (window.attachEvent) window.attachEvent("onload", addAdalFrame);
//else window.onload = addAdalFrame;
    ...
    AdalModule.factory('TokenService', function ($rootScope, $q) {
        ...
        return {
            ...
            getUser: function () {
                init();
                var deferred = $q.defer();
                // make Graph call to return info
...
// =============       Export       ===============
// Exports module to run tests in node.js
