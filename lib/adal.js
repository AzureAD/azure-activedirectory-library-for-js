...
AuthenticationContext.prototype._renewIdToken = function (callback) {
    ...
    if (window.parent.loadframeStatus[this.config.clientId] === this.CONSTANTS.LOADFRAME_PENDING) {
        return;
    }
    ...
    this._logstatus('Renew ID token Expected state: ' + expectedState);
...
/* Calling _loadFrame but with a timeout to signal failure in loadframeStatus */
AuthenticationContext.prototype._loadFrameTimeout = function (urlNavigation, frameName, resource, timeout) {
    //set iframe session to pending
    this._logstatus('Set loading state to pending for: ' + resource);
    ...
    setTimeout(function () {
        if (window.loadframeStatus[resource] === self.CONSTANTS.LOADFRAME_PENDING) {
            ...
            self._logstatus('Loading frame has timed out after: ' + timeoutDuration + ' seconds for resource ' + resourceSaved);
            window.loadframeStatus[resource] = self.CONSTANTS.LOADFRAME_EXPIRED;
        }
    }, timeout);
}
...
AuthenticationContext.prototype.acquireToken = function (resource, callback) {
    if (this._isEmpty(resource)) {
        callback('acquireToken: resource is required', null);
        return;
    }
    ...
    if (this._getItem(this.CONSTANTS.STORAGE.FAILED_RENEW)) {
        this._logstatus('acquireToken: renewToken is failed:' + this._getItem(this.CONSTANTS.STORAGE.FAILED_RENEW));
        callback(this._getItem(this.CONSTANTS.STORAGE.FAILED_RENEW), null);
        return;
    }

    if (!this._user) {
        callback('acquireToken: User login is required', null);
        return;
    }
    ...
    if ((this._activeRenewals[resource] || window.parent.AuthenticationContext()._activeRenewals[resource])
        && (window.loadframeStatus[resource] !== this.CONSTANTS.LOADFRAME_EXPIRED || window.parent.loadframeStatus[resource] !== this.CONSTANTS.LOADFRAME_EXPIRED)) {
        //Active renewals contains the state for each renewal.
        this.registerCallback(this._activeRenewals[resource], resource, callback);
        this._logstatus('acquireToken: register callback for ' + resource);
    }
    else {
        if (resource === this.config.clientId) {
            // App uses idtoken to send to api endpoints
            // Default resource is tracked as clientid to store this token
            this._logstatus('acquireToken: renewing idtoken');
            this._renewIdToken(callback);
        } else {
            this._logstatus('acquireToken: renewing token ' + resource);
    ...
    if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION)) {
        ...
        if (requestInfo.stateMatch) {
            ...
            if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ACCESS_TOKEN)) {
                ...
                this._logstatus('token sucessfully updated in local storage. removing iframe session tracking for ' + resource);
            }
...
AuthenticationContext.prototype.handleWindowCallback = function () {
    ...
    if (this.isCallback(hash)) {
        ...
        if ((requestInfo.requestType === this.REQUEST_TYPE.RENEW_TOKEN ||
            ...
            this._logstatus('iframe came back, deleting iframe session tracking for ' + requestInfo.stateResponse);
            window.src = '';
