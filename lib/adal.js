...
AuthenticationContext.prototype.login = function () {
    ...
    this._logstatus('Expected state: ' + expectedState + ' _startPage:' + window.location);
...
AuthenticationContext.prototype.getCachedUser = function () {
    return this._getItem(this.CONSTANTS.STORAGE.USERNAME);
};
...
AuthenticationContext.prototype.isRenewingToken = function () {
    return this._getItem(this.CONSTANTS.STORAGE.STATE_RENEW);
};
...
AuthenticationContext.prototype._renewToken = function (resource, callback) {
    ...
    this._saveItem(this.CONSTANTS.STORAGE.STATE_RENEW_RESOURCE, resource);
    this._saveItem(this.CONSTANTS.STORAGE.FAILED_RENEW, '');
...
AuthenticationContext.prototype.logOut = function () {
    ...
    if (this.config.postLogoutRedirectUri) {
        logout = 'post_logout_redirect_uri=' + this.config.postLogoutRedirectUri;
    }
...
AuthenticationContext.prototype.getUser = function (callback) {
    ...
    if (token) {
        ...
        var userInStorage = this._getItem(this.CONSTANTS.STORAGE.USERNAME);
        if (!this._isEmpty(userInStorage)) {
            this._logstatus('User exists in cache: ' + userInStorage);
        ...
        if (this._getItem(this.CONSTANTS.STORAGE.FAILED_RENEW)) {
            this._logstatus('renewToken is failed:' + this._getItem(this.CONSTANTS.STORAGE.FAILED_RENEW));
            this.callback(this._getItem(this.CONSTANTS.STORAGE.FAILED_RENEW), null);
            return;
        }

        this._getIdTokenInFrame(callback);
    } else {
        this._logstatus('User is not authorized');
        this.callback(this.CONSTANTS.ERR_MESSAGES.NO_TOKEN, null);
    }
};
...
AuthenticationContext.prototype._getIdTokenInFrame = function (callback) {
    ...
    this._saveItem(this.CONSTANTS.STORAGE.STATE_IDTOKEN, expectedState);
    this._saveItem(this.CONSTANTS.STORAGE.USERNAME, '');
        ...
        if (parameters.hasOwnProperty('error_description') ||
            ...
            switch (stateResponse) {
                case this._getItem(this.CONSTANTS.STORAGE.STATE_LOGIN):
                    ...
                    break;
                case this._getItem(this.CONSTANTS.STORAGE.STATE_RENEW):
        ...
        if (requestInfo.stateMatch) {
            ...
            if (requestInfo.parameters.hasOwnProperty('access_token')) {
                ...
                if (requestInfo.requestType === this.REQUEST_TYPE.RENEW_TOKEN) {
                    resource = this._getItem(this.CONSTANTS.STORAGE.STATE_RENEW_RESOURCE);
                }
            ...
            if (requestInfo.parameters.hasOwnProperty('id_token')) {
                this._logstatus('Fragment has IdToken');
                this._saveItem(this.CONSTANTS.STORAGE.USERNAME, this._extractUserName(requestInfo.parameters['id_token']));
            }
        } else {
...
/*exported handleWindowCallback, oauth2Callback */
...
AuthenticationContext.prototype._guid = function () {
   ...
   //     values.
   //UUID                   = time-low "-" time-mid "-"time-high-and-version "-"clock-seq-and-reserved"-" node
   //time-low               = 4hexOctet
   //time-mid               = 2hexOctet
   ...
   //node                   = 6hexOctet
   // y could be 1000, 1001, 1010, 1011 remaining hex for part of the hexoctet is random
   // y values are 8, 9, A, B
    ...
    for (var i = 0; i < 36; i++) {
        ...
        if (guidHolder[i] === 'x') {
            ...
            r = (r & 0x3 | 0x8); // mask 0011 with and to set to zero and mask 1000 or to set to 1
            guidHolder[i] = hex[r];
        }
    }
    ...
    var AdalService = function () {
        ...
        this.init = function (configOptions, httpProvider) {
            if (configOptions) {
                ...
                configOptions.redirectUri = configOptions.redirectUri || window.location.href;
                configOptions.postLogoutRedirectUri = configOptions.postLogoutRedirectUri || window.location.href;
        ...
        this.$get = function ($rootScope, $window, $http, $q, $location, $timeout) {
            ...
            var locationChangeHandler = function (event, newUrl, oldUrl) {
                ...
                console.log('Location change event newurl:' + newUrl + ' oldurl:' + oldUrl);
                    ...
                    if (requestInfo.stateMatch) {
                        if (typeof _adal.callback === 'function') {
                            ...
                            if (requestInfo.requestType === _adal.REQUEST_TYPE.RENEW_TOKEN) {
                                ...
                                return;
                            } else if (requestInfo.requestType === _adal.REQUEST_TYPE.ID_TOKEN) {
                                _adal.callback(_adal._getItem(_adal.CONSTANTS.STORAGE.ERROR_DESCRIPTION), _adal._getItem(_adal.CONSTANTS.STORAGE.USERNAME));
                                return;
                            }
                        } else {
