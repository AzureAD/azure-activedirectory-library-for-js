    ...
    if (requestInfo.parameters.hasOwnProperty('error_description')) {
        ...
        if (requestInfo.stateMatch) {
            ...
            if (requestInfo.parameters.hasOwnProperty('id_token')) {
                ...
                var name = this._extractUserName(requestInfo.parameters['id_token']);
                if(name){
                    this._saveItem(this.CONSTANTS.STORAGE.USERNAME, name);
                }
            }
        } else {
...
function handleWindowCallback(hash) {
    ...
    if (window.parent && window.parent.oauth2Callback) {
        ...
        window.parent.setTimeout(window.parent.oauth2Callback(hash), 1);
        window.src = '';
    } else if (window && window.oauth2Callback) {
        console.log('Window is redirecting');
        window.setTimeout(window.oauth2Callback(hash), 1);
    }
}

function oauth2Callback(hash) {
