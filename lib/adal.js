...
AuthenticationContext = function (config) {
    ...
    */
    ...
    this._valiateInputScope(this.config.scope);
};
...
AuthenticationContext.prototype.login = function (additionalScope) {
    ...
    if (!additionalScope || this._isEmpty(additionalScope)) {
        scope = this.config.scope;
    }
    else {
        scope = this.config.scopes.concat(additionalScope);
        this._valiateInputScope(scope);
    }
...
AuthenticationContext.prototype._hasEntry = function (entry) {
    ...
    return keys && !this._isEmpty(keys) && (keys.indexOf(entry) > -1);
}


AuthenticationContext.prototype.getCachedToken = function (scopes) {
    this._valiateInputScope(scopes);

    var targetEntryKey;

    // For get cachedtoken, if it's for id token
    if (this._isEmpty(scopes) || (scopes.indexOf(this.config.clientId) > -1)) {
        targetEntryKey = this.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY + this.config.clientId;
    }
    else { 
        targetEntryKey = this._createTokenCacheEntryKey();
    }

    if (!this._hasEntry(targetEntryKey)) {
        return null;
    }
    
    var valueInCache = this._getItem(targetEntryKey);
    if (this._scopeContains(valueInCache[this.CONSTANTS.PARAMETERS.SCOPE], scopes)) {
        
        var entryKey = this._createTokenCacheEntryKey();
        var value = this._getItem(entryKey);
        var token = value[this.CONSTANTS.PARAMETERS.TOKEN];
        var expired = value[this.CONSTANTS.PARAMETERS.EXPIRE];
...
AuthenticationContext.prototype._renewToken = function (scopes, callback) {
    ...
    if (!this._hasEntry(this._createTokenCacheEntryKey())) {
        var keys = this._getItem(this.CONSTANTS.STORAGE.TOKEN_KEYS) || [];
        this._saveItem(this.CONSTANTS.STORAGE.TOKEN_KEYS, keys.push(this._createTokenCacheEntryKey()));
    }
...
AuthenticationContext.prototype._renewIdToken = function (callback) {
    ...
    if (!this._hasEntry(this.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY + this.config.clientId)) {
        var keys = this._getItem(this.CONSTANTS.STORAGE.TOKEN_KEYS) || [];
        this._saveItem(this.CONSTANTS.STORAGE.TOKEN_KEYS, keys.push(this.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY + this.config.clientId));
    }
    ...
    if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION)) {
        ...
        if (requestInfo.stateMatch) {
            ...
            if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ACCESS_TOKEN)) {
                ...
                //TODO: save token with returned scopes. 
                var entryKey = this._createTokenCacheEntryKey();
                var entryValue = this._createTokenCacheEntryValue(requestInfo);
                
                if (!this._hasEntry(entryKey)) {
                    var keys = this._getItem(this.CONSTANTS.STORAGE.TOKEN_KEYS) || [];
                    this._saveItem(this.CONSTANTS.STORAGE.TOKEN_KEYS, keys.push(entryKey));
                }
            ...
            if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ID_TOKEN)) {
                ...
                if (this._user && this._user.profile) {
                    if (this._user.profile.nonce !== this._getItem(this.CONSTANTS.STORAGE.NONCE_IDTOKEN)) {
                        ...
                        var entry = this.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY + this.config.clientId;
                        if (!this._hasEntry(entry)) { 
                            var keys = this._getItem(this.CONSTANTS.STORAGE.TOKEN_KEYS) || [];
                            this._saveItem(this.CONSTANTS.STORAGE.TOKEN_KEYS, keys.push(entry));   
                        }

                        this._saveItem(entry, this._createTokenCacheEntryValue(requestInfo));
                    }
                }
            }
        } else {
...
AuthenticationContext.prototype._createTokenCacheEntryKey = function () {
    ...
    if (this.config.tenant) {
        key[this.CONSTANTS.PARAMETERS.TENANT] = this.config.tenant;
    }
...
AuthenticationContext.prototype._createTokenCacheEntryValue = function (requestInfo) {
    ...
    if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ID_TOKEN)) {
        value[this.CONSTANTS.PARAMETERS.TOKEN] = request.parameters[this.CONSTANTS.ID_TOKEN];
        ...
        value[this.CONSTANTS.PARAMETERS.TOKEN] = request.parameters[this.CONSTANTS.ACCESS_TOKEN];
...
AuthenticationContext.prototype._valiateInputScope = function (scopes) {
    ...
    console.log('contains openid' + scopes.indexOf('openid'));
    if (scopes.indexOf('openid') > -1) {
