...
Adal.prototype.getUser = function (callback) {
    // get cached token
    var token = this.getCachedToken(this.config.resource);
    this.callback = callback;
    if (token) {
                ...
                if (_adal.isCallback(hash)) {
                    ...
                    if (requestInfo.stateMatch) {
                        if (typeof _adal.callback === 'function') {
                            ...
                            updateDataFromCache(_adal.config.resource);
                            var token = _adal.getCachedToken(_adal.config.resource);
