...
AuthenticationContext.prototype._loadFrame = function (urlNavigate, frameName) {
    ...
    setTimeout(function () {
        ...
        if (frameHandle.src === '' || frameHandle.src === 'about:blank') {
			...
			// Build correct listener for receiving token
			...
			eventer(messageEvent, function(e) {
				...
				// Save token
				var ri = self.getRequestInfo(data);
				self.saveTokenFromHash(ri);

				self.callback(null, self.getCachedToken(self.config[frameName === "adalIdTokenFrame"?"clientId":"loginResource"]));
			});
		}
    }, 500);
};
