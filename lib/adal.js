...
var AuthenticationContext = (function () {
    ...
    AuthenticationContext = function (config) {
        ...
        window.requestType = this.REQUEST_TYPE.LOGIN;
  ...
  */
    ...
    /**
     * Adds scope to authorization URL determined for the resource. This is specified by assigning an endpoint to an object
     * Example: endpoint['http://hostapp.com/api'] = { resource: 'hostapp.my-sharepoint.com', scope: ['openid', 'profile'] };
     * @ignore
     */
    AuthenticationContext.prototype._addScopeParameters = function(urlNavigate, resource) {
        var scope = this._getEndpointByResource(resource).scope;

        if (scope.length > 0) {
            urlNavigate += '&scope=' + encodeURIComponent(scope.join(' '));
        }
    ...
    /**
     * Returns a object describing the endpoint with given resource
     * @ignore
     */
    AuthenticationContext.prototype._getEndpointByResource = function(resource) {
        var endpointObject = { resource: resource, scope: [] };

        if (this.config && this.config.endpoints) {
            for (var configEndpoint in this.config.endpoints) {
                let endpoint = this.config.endpoints[configEndpoint];
                if (endpoint.resource && endpoint.resource === resource) {
            ...
            if (parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION) ||
                ...
                if (!requestInfo.stateMatch && window.parent) {
                    if (window.parent !== window) { //iframe
                        requestInfo.authenticationMode = this.AUTHENTICATION_MODES.IFRAME;
                        requestInfo.requestType = this.REQUEST_TYPE.RENEW_TOKEN;
                    }
                    else { //ATPopup or LOGINPopup
                        requestInfo.authenticationMode = this.AUTHENTICATION_MODES.POPUP;
                        requestInfo.requestType = window.parent.requestType;
                    }
    ...
    */
        ...
        if (loginStates) {
            ...
            for (var i = 0; i < loginStates.length; i++) {
                if (loginStates[i] === requestInfo.stateResponse) {
                    ...
                    requestInfo.stateMatch = true;
                    requestInfo.authenticationMode = this.AUTHENTICATION_MODES.REDIRECT;
                    return true;
                }
            }
        }
        ...
        if (this.config && this.config.endpoints) {
            for (var configEndpoint in this.config.endpoints) {
                ...
                if (endpoint.indexOf(configEndpoint) > -1) {
                    let matchingEndpoint = this.config.endpoints[configEndpoint];
                    if (matchingEndpoint.resource && matchingEndpoint.resource.length > 0) {
                        return matchingEndpoint.resource;
                    } else {
    ...
    AuthenticationContext.prototype.handleWindowCallback = function (hash) {
        ...
        if (this.isCallback(hash)) {
            ...
            if (requestInfo.authenticationMode === this.AUTHENTICATION_MODES.REDIRECT) {
