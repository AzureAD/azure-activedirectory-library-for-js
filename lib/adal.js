 ...
 * @param {string} policy The policy that the token is acquired for. 
 */
AuthenticationContext.prototype.login = function (additionalScope, policy) {
    var scope = [];
    ...
    if (this._isEmpty(policy)) {
        policy = '';
    }
    else {
        this._saveItem(this.CONSTANTS.STORAGE.POLICY, policy);
    }
    ...
    var urlNavigate = this._getNavigateUrl('id_token', scope, policy) + '&nonce=' + encodeURIComponent(this._idTokenNonce);
...
AuthenticationContext.prototype.getCachedToken = function (scopes, policy) {
    this._validateInputScope(scopes);
    policy = this._isEmpty(policy) ? '' : policy;
    ...
    if (this._isEmpty(scopes) || (scopes.indexOf(this.config.clientId) > -1)) {
        targetEntryKey = this._createTokenCacheEntryKeyForIDToken(policy);
        potentialEntries = this._getPotentialEntries(targetEntryKey);
    }
    else {
        targetEntryKey = this._createTokenCacheEntryKey(policy);
    ...
    if (potentialEntries.length == 0) {
        ...
        Logging.log('scopes: ' + scopes + ';' + 'policy: ' + policy);
...
* @param   {string} policy policy that the access token is acquired for. 
* @returns {string} access token if request is successfull
*/
AuthenticationContext.prototype._renewToken = function (scopes, policy, callback) {
    ...
    var urlNavigate = this._getNavigateUrl('token', scopes, policy) + '&prompt=none&login_hint=' + encodeURIComponent(this._user.userName);
    ...
    this._log(parsedScopes, 'Navigate to:' + urlNavigate);
...
AuthenticationContext.prototype._renewIdToken = function (scopes, policy, callback) {
    // use iframe to try refresh token
    this._logstatus('renewIdToken is called');

    if (!this._hasEntry(this._createTokenCacheEntryKeyForIDToken(policy))) {
        var keys = JSON.parse(this._getItem(this.CONSTANTS.STORAGE.TOKEN_KEYS)) || [];
        keys.push(this._createTokenCacheEntryKeyForIDToken(policy));
        this._saveItem(this.CONSTANTS.STORAGE.TOKEN_KEYS, JSON.stringify(keys));
    }
    ...
    var urlNavigate = this._getNavigateUrl('id_token', scopes, policy) + '&prompt=none&login_hint=' + encodeURIComponent(this._user.userName);
    urlNavigate += '&domain_hint=' + encodeURIComponent(this._getDomainHint());
    urlNavigate += '&nonce=' + encodeURIComponent(this._idTokenNonce);
    this.callback = callback;
    this.idTokenNonce = null;
...
* @param {string}  policy  The policy to acquire token with. 
* @param {requestCallback} callback 
* @note scopes for this method is already consented. 
*/
AuthenticationContext.prototype.acquireTokenSilent = function (scopes, policy, callback) {
    scopes = scopes || this.config.scope;
    this._validateInputScope(scopes);
    
    window.policy = this._isEmpty(policy) ? '' : policy;
    
    var token;
    try {
        token = this.getCachedToken(scopes, policy);
    }
    catch (e) {
        callback(e, null);
    }
    ...
    if (this._isEmpty(scopes) || (scopes.indexOf(this.config.clientId) > -1)) {
        ...
        this._renewIdToken(scopes, window.policy, callback);
    } else {
        this._logstatus('renewing accesstoken');
        this._renewToken(scopes, window.policy, callback);
    }
};
...
AuthenticationContext.prototype.clearCacheForStoredEntry = function (scopes, policy) {
    this._saveItem(this.CONSTANTS.STORAGE.FAILED_RENEW, '');
    ...
    if (scopes=== this.config.clientId || scopes && scopes.indexOf(this.config.clientId) > -1) {
        entryKeyToBeRemoved = this._createTokenCacheEntryKeyForIDToken(policy);
        //If policy is empty, should remove all the entries that match authority, clientid or username. 
  
    }
    else {
        entryKeyToBeRemoved = this._createTokenCacheEntryKey(policy);
    }
    ...
    if (this._isEmpty(policy)) {
        ...
        this._saveItem(entryKeyToBeRemoved, JSON.stringify({}));
    }
};
 ...
 * @param {string} hash  -  Hash passed from redirect page
 * @param {string} policy - The policy to be stored into cache as part of the cache key. 
 * @returns {string} error message related to login
 */
AuthenticationContext.prototype.saveTokenFromHash = function (requestInfo, policy) {
    ...
    if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION)) {
        ...
        if (requestInfo.stateMatch) {
            ...
            if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ACCESS_TOKEN)) {
                ...
                var entryKey = this._createTokenCacheEntryKey(policy);
            ...
            if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ID_TOKEN)) {
                ...
                if (this._user && this._user.profile) {
                    if (this._user.profile.nonce !== this._getItem(this.CONSTANTS.STORAGE.NONCE_IDTOKEN)) {
                        ...
                        var entryKey = this._createTokenCacheEntryKeyForIDToken(policy);
                        var entryValue = this._createTokenCacheEntryValue(requestInfo);
...
/**
 * Extract the policy for givien endpoint. 
 * @param {Hash} endpoint The endpoint that are requested for policy. 
 * @returns {string} policy for the endpoint. 
 */
AuthenticationContext.prototype.getPolicyForEndpoint = function (endpoint) {
    if (this.config && this.config.endpoints) {
        for (var configEndpoint in this.config.endpoints) {
            // configEndpoint is like /api/Todo requested endpoint can be /api/Todo/1
            if (endpoint.indexOf(configEndpoint) > -1) {
                var endpointEntry = this.config.endpoints[configEndpoint];
                if (endpointEntry && endpointEntry.hasOwnProperty('policy')) {
                    return endpointEntry['policy'];
                }
            }
        }
    }
...
AuthenticationContext.prototype.handleWindowCallback = function () {
    ...
    if (this.isCallback(hash)) {
        ...
        this._log(this._getScopesFromState(requestInfo.stateResponse), "returned from redirect url");
        var policy = requestInfo.requestType === this.REQUEST_TYPE.LOGIN ? this._getItem(this.CONSTANTS.STORAGE.POLICY) : window.parent.policy;
        this.saveTokenFromHash(requestInfo, policy);
        var callback = null;
...
AuthenticationContext.prototype._getNavigateUrl = function (responseType, scopes, policy) {
    var tenant = 'common';
    if (this.config.tenant) {
        tenant = this.config.tenant;
    }
    ...
    var urlNavigate = this.instance + tenant + '/oauth2/v2.0/authorize' + this._serialize(responseType, this.config, scopes, policy) + this._addClientId();
...
AuthenticationContext.prototype._serialize = function (responseType, obj, scopes, policy) {
    var str = [];
    if (obj !== null) {
        ...
        str.push('slice=testslice&msaproxy=true&api-version=1.0');
        ...
        if (!this._isEmpty(policy)) {
            str.push('p=' + encodeURIComponent(policy));
        }
    }
...
AuthenticationContext.prototype._log = function (scopes, message) {
    var correlationId = this.config.correlationId;
    var timestamp = new Date().toUTCString();
    
    var formattedMessage = timestamp + ':' + correlationId + ': ' + scopes + ':' + message + ';';
...
AuthenticationContext.prototype._scopeContains = function (scopeInCache, scopeToStore) {
    ...
    for (var index = 0; index < scopeToStore.length; index++) {
        if (scopeInCache.indexOf(scopeToStore[index]) == -1) {
            return false;
        }
    }
 ...
 * @param {string} policy The optional value used to create access token storage key.  
 * @returns {string} The access token cache storage key. 
 */
AuthenticationContext.prototype._createTokenCacheEntryKey = function (policy) {
    var key = {};
    ...
    if (!this._isEmpty(policy)) {
        key[this.CONSTANTS.PARAMETERS.POLICY] = policy;
    }
 ...
 * @param {string} policy The optional value used to create id token storage key.  
 * @returns {string} The id token cache storage key. 
 */
AuthenticationContext.prototype._createTokenCacheEntryKeyForIDToken = function (policy) {
    var key = {};
...
AuthenticationContext.prototype._createTokenCacheEntryValue = function (requestInfo) {
    ...
    if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.PARAMETERS.SCOPE)) {
        value[this.CONSTANTS.PARAMETERS.SCOPE] = requestInfo.parameters[this.CONSTANTS.PARAMETERS.SCOPE];
    }
