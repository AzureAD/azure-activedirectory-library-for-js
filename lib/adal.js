...
var AuthenticationContext = (function () {
    ...
    AuthenticationContext = function (config) {
        ...
        this.CONSTANTS = {
            ...
            ERROR: 'error',
            STORAGE: {
                TOKEN_KEYS: 'msid.token.keys',
                ACCESS_TOKEN_KEY: 'msid.access.token.key',
                EXPIRATION_KEY: 'msid.expiration.key',
                STATE_LOGIN: 'msid.state.login',
                STATE_RENEW: 'msid.state.renew',
                NONCE_IDTOKEN: 'msid.nonce.idtoken',
                SESSION_STATE: 'msid.session.state',
                IDTOKEN: 'msid.idtoken',
                ERROR: 'msid.error',
                ERROR_DESCRIPTION: 'msid.error.description',
                LOGIN_REQUEST: 'msid.login.request',
                LOGIN_ERROR: 'msid.login.error',
                RENEW_STATUS: 'msid.token.renew.status',
                ANGULAR_LOGIN_REQUEST: 'msid.angular.login.request'
            },
            DEPRECATED_STORAGE_IDTOKEN: 'adal.idtoken',
            DEPRECATED_STORAGE_TOKEN_KEYS: 'adal.token.keys',
            DEPRECATED_STORAGE_PREFIX: 'adal',
            RESOURCE_DELIMETER: '|',
            CACHE_DELIMETER: '||',
    ...
    */
    ...
    AuthenticationContext.prototype.getCachedUser = function () {
        ...
        var idtoken = this._getCachedIdToken();
  ...
  */
    ...
    AuthenticationContext.prototype._createUser = function (idToken) {
        ...
        if (parsedJson && parsedJson.hasOwnProperty('aud')) {
            if (parsedJson.aud.toLowerCase() === this.config.clientId.toLowerCase()) {
                ...
                } else if (parsedJson.hasOwnProperty('preferred_username')) {
    ...
    */
     ...
     * @param {*} key 
     */
    AuthenticationContext.prototype._removeItem = function (key) {
    ...
    /**
     * Rename the key for an item in storage
     * @param {string} fromKey - The original key name.
     * @param {string} toKey - The new key name.
     */
    AuthenticationContext.prototype._renameItem = function(fromKey, toKey) {
        var currentValue = this._getItem(fromKey);

        this._saveItem(toKey, currentValue);
        this._removeItem(fromKey);
        this.verbose('Renamed storage item key from ' + fromKey + ' to ' + toKey);  
    };
    ...
    /**
     * Upgrade the storage keys if they have deprecated key prefix
     * @ignore
     */
    AuthenticationContext.prototype._upgradeStorage = function() {
        this.log(this.CONSTANTS.LOGGING_LEVEL.INFO, 'Upgrading cached storage keys.', null);

        // access token resource keys
        var resourceKeysString = this._getItem(this.CONSTANTS.DEPRECATED_STORAGE_TOKEN_KEYS);
        var resourceKeys = [];
        if (!this._isEmpty(resourceKeysString)) {
            resourceKeys = resourceKeysString.split(this.CONSTANTS.RESOURCE_DELIMETER);
        }

        // update all the deprecated keys
        for(var key in this.CONSTANTS.STORAGE) {
            var storageKey = this.CONSTANTS.STORAGE[key];
            var storageKeyParts = storageKey.split('.');
            if (storageKeyParts.length > 0) {
                storageKeyParts[0] = this.CONSTANTS.DEPRECATED_STORAGE_PREFIX;
                var deprecatedStorageKey = storageKeyParts.join('.');

                // check for the key by itself
                if (this._getItem(deprecatedStorageKey) !== null) {
                    this._renameItem(deprecatedStorageKey, storageKey);
                } else {
                    // check for access token keys with resource appended
                    for(var resourceKey in resourceKeys) {
                        var resourceKeyValue = resourceKeys[resourceKey];
                        var deprecatedStorageResourceKey = deprecatedStorageKey + resourceKeyValue;
                        var storageResourceKey = storageKey + resourceKeyValue;
                        if (this._getItem(deprecatedStorageResourceKey) !== null) {
                            this._renameItem(deprecatedStorageResourceKey, storageResourceKey);
                        }
                    }
                }
            }
    ...
    /**
     * Return the cached id token.  
     * Also, if the token is not found checks to see if the older token exists
     * exist from either ADAL does an update to storage and returns the token.
     * @ignore
     */
    AuthenticationContext.prototype._getCachedIdToken = function () {
        ...
        // if the old token exists upgrade to the latest storage naming.
        var deprecatedIdToken = this._getItem(this.CONSTANTS.DEPRECATED_STORAGE_IDTOKEN);
        if (!this._isEmpty(deprecatedIdToken)) {
            this._upgradeStorage();
        }
