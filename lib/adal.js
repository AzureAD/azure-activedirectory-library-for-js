    ...
    AuthenticationContext.prototype._loginPopup = function (urlNavigate, resource, callback) {
        ...
        var pollTimer = window.setInterval(function () {
            if (!popupWindow || popupWindow.closed || popupWindow.closed === undefined) {
                ...
                that.warn('Popup window closed by UI action');
                that._saveItem(that.CONSTANTS.STORAGE.ERROR, 'Popup closed');
                that._saveItem(that.CONSTANTS.STORAGE.ERROR_DESCRIPTION, 'Popup window closed by UI action');
                that._saveItem(that.CONSTANTS.STORAGE.LOGIN_ERROR, 'Popup window closed by UI action');
                if (loginCallback)
                    loginCallback(that._getItem(that.CONSTANTS.STORAGE.ERROR_DESCRIPTION), null, that._getItem(that.CONSTANTS.STORAGE.ERROR));
            }
            try {
