  ...
  */
    ...
    */
    ...
    AuthenticationContext.prototype._getResourceFromState = function (state) {
        if (state) {
            var splittedInfo = state.split('|');

            if (splittedInfo.length > 1) {
                return splittedInfo[1];
            }
        }
    ...
    /**
     * Extracts token expiration base time value from state.
     * @ignore
     */
    AuthenticationContext.prototype._getExpireBaseTimeFromState = function (state) {
        if (state) {
            ...
            if (splittedInfo.length > 2) {
                return parseInt(splittedInfo[2], 10);
            }
        }
        return 0;
    };
    ...
    AuthenticationContext.prototype._expiresIn = function (expires, state) {
        ...
        var expireBaseFromState = this._getExpireBaseTimeFromState(state)
        if (expireBaseFromState === 0) {
            return this._now() + parseInt(expires, 10);
        } else {
            return expireBaseFromState + parseInt(expires, 10);
        }
    };
