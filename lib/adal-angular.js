        ...
        AdalModule.provider('adalAuthenticationService', function () {
            ...
            this.$get = ['$rootScope', '$window', '$q', '$location', '$timeout','$injector', function ($rootScope, $window, $q, $location, $timeout, $injector) {
                ...
                var routeChangeHandler = function (e, nextRoute) {
                    if (!_paths) {
                        _paths = $injector.get('$route').routes;
                        for (var route in _paths) {
                            if (_paths[route].hasOwnProperty('templateUrl') && !isADLoginRequired(_paths[route], _adal.config)) {
                                if (!(_adal.config.anonymousEndpoints.indexOf(_paths[route].templateUrl) > -1)) {
                                    _adal.config.anonymousEndpoints.push(_paths[route].templateUrl);
                                }
                            }
                        }
                    }
                ...
                var stateChangeHandler = function (e, toState, toParams, fromState, fromParams) {
                    if (!_paths) {
                        _paths = $injector.get('$state').get();
                        _paths.forEach(function (state) {
                            if (state.templateUrl && !isADLoginRequired(state, _adal.config)) {
                                if (!(_adal.config.anonymousEndpoints.indexOf(state.templateUrl) > -1))
                                    _adal.config.anonymousEndpoints.push(state.templateUrl);
                            }
                        });
                    }
                    if (toState) {
